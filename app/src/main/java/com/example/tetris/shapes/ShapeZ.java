...
public class ShapeZ extends Piece {
    public ShapeZ(int xMin, int yMin, int direction) {
        super(xMin, yMin);
    ...
    @Override
    public void rotate() {
        ShapeZ[] temp = new ShapeZ[2];
        if(direction == 0) {
            temp[0] = new ShapeZ(xMin, yMin - 1, 1);
            temp[1] = new ShapeZ(xMin, yMin, 1);
        }
        if(direction == 1) {
            temp[0] = new ShapeZ(xMin, yMin + 1, 0);
            temp[1] = new ShapeZ(xMin - 1, yMin + 1, 0);
        }
        for(int i = 0; i < 2; i++) {
